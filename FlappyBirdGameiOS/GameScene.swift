...
class GameScene: SKScene {
    ...
    override func sceneDidLoad() {
        ...
        let groundTextureWidth = self.frame.width * 2.0
        ...
        for i in 0 ..< 2 + Int(self.frame.size.width / (self.frame.width / 4.0) * 2.0 ) {
            let sprite = SKSpriteNode(texture: groundTexture)
//            sprite.setScale(0.5)
            sprite.size = CGSize(
                width: self.frame.width,
                height: self.frame.width / 366 * 112
            )
...
//        let skyTextureWidth = WKInterfaceDevice.current().screenBounds.width * 2
        let skyTextureWidth = self.frame.width
...
//            sprite.setScale(0.5)
            sprite.size = CGSize(
                width: self.frame.width,
                height: self.frame.width / 1668 * 1180
            )
            sprite.zPosition = -20
            sprite.position = CGPoint(
                x: CGFloat(i) * sprite.size.width,
                y: self.frame.width / 1668 * 1180 / 2 + self.frame.width / 366 * 112 / 2 / 1.2
            )
        ...
        // setup our bird
        let birdTexture1 = SKTexture(image: UIImage(named: "cloud-01", in: Bundle(identifier: "com.mengdongfuture.FlappyBirdGameiOS"), with: nil)!)
        birdTexture1.filteringMode = .nearest
        let birdTexture2 = SKTexture(image: UIImage(named: "cloud-02", in: Bundle(identifier: "com.mengdongfuture.FlappyBirdGameiOS"), with: nil)!)
